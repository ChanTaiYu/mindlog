...
const IMGW = 165, IMGH = Math.round(165 * 1400 / 630); // keep 630x1400 aspect
  ...
  return [
    new Paragraph({
      ...
      children: [new ImageRun({
        type: 'png', data: fs.readFileSync(path.join(DOCS, file)),
...
// Two screenshots side by side in a borderless table (saves vertical space)
...
const features = [
  ...
  ['Diary entries', 'Create, edit and delete journal entries with a title, body, date, mood and an optional photo from the camera or gallery.'],
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      P('Each module below is labelled with its developer (Chan Tai Yu for all modules) and illustrated with screenshots captured from the app running on an Android emulator.'),
      ...
      ...figurePair('shot_diary.png', 'Diary list of entries', 'shot_editor.png', 'New entry editor with mood picker'),
      ...
      P('The task planner is a lightweight to-do list. Users add a task with a single text field, tick it off when complete, and swipe to delete. A summary line shows how many tasks are open versus done. Crucially, each completed task records the time it was completed, which feeds the mood–productivity insight described next.'),
      ...figure('shot_tasks.png', 'Task planner with completed and outstanding tasks'),
      ...
      ...figure('shot_insights.png', 'Insights screen relating mood to tasks completed'),
      ...
      P('Robust and responsive. Analytics such as the mood trend and the insight chart are computed from data already held in memory, so screens update instantly without extra database round-trips. The codebase passes static analysis with no issues and is covered by unit tests for the security key-derivation logic and the data models, giving confidence in the most critical code.'),
      ...
      P('I also grew more comfortable with Flutter’s widget model and state management using Provider, and with integrating plugins for biometrics, the camera, notifications and HTTP. Debugging was a real learning experience: I encountered layout errors caused by nesting a Scaffold inside an IndexedStack and by a button whose theme forced an infinite width inside a row. Tracking these down from the framework’s error output, understanding why the constraints were invalid, and fixing them taught me far more than writing the happy path did.'),
      P('If I were to start again, I would plan the persistence and analytics layer up front so that screens compute from in-memory data from the beginning, and I would set up a physical-device test loop earlier. Overall I am proud that the app is both feature-rich and genuinely secure, and I finish the module much more confident in mobile development.'),
